...
public class qtyStepper extends JPanel implements Serializable {
    ...
    protected void paintComponent(Graphics g) {
        ...
        int fillArc = Math.min(radius * 2, Math.min(w, h));
        RoundRectangle2D fillShape = new RoundRectangle2D.Double(0, 0, w, h, fillArc, fillArc);
        ...
        if (borderSize > 0) {
            double offset = borderSize / 2.0;
            RoundRectangle2D drawShape = new RoundRectangle2D.Double(
                    offset, 
                    offset, 
                    w - borderSize, 
                    h - borderSize, 
